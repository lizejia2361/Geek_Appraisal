...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   Huffman树是所谓的正则二叉树，只有度为0和度为2的**结**点 根据二叉树的性质，**n**0 = n2 + 1，因此该**树**中**度**为2结点数量**为**n-1 于是**一**共有2n-1个结点

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

3. 使用画图工具画出上述两个Huffman生成树的生成过程

+![245f9e380bae55520f03c2ad26ad70f](C:\Users\ASUS\Desktop\245f9e380bae55520f03c2ad26ad70f.jpg)